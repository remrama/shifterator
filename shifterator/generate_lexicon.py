...
def DDR(model, ddr_name, ddr_words):
    ...
    export_scores_fname = os.path.join(EXPORT_DIR, f"DDR_{ddr_name}-{model}.tsv")
    ...
    ddr_vector = np.mean([ word2vec.get_vector(w) for w in ddr_words ], axis=0)
